      ...
      <div class="flex flex-col items-center mt-4 px-6 space-y-4 md:flex-row md:space-y-0 md:space-x-8">
        ...
        >
          <select-group
            ...
            v-model="editableSearchOptions.category_id"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    confirmDelete() {
      if (this.isLoadingDelete) {
        return this.$errorToast(
          "It's only possible to delete one blogs at a time."
        );
      }
      this.$inertia.delete(
        this.$route("admin.cms.blogs.destroy", this.rowToDelete.id),
        {
          only: ["flash", "blogs"],
        }
      );
    ...
    onSearchOptionsUpdate: _.debounce(function () {
      ...
      Inertia.get(
        this.$route("admin.cms.blogs.index"),
